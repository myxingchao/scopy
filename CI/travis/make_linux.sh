...
cd ${TRAVIS_BUILD_DIR}/build
cmake -DENABLE_STATIC_LINKING=ON -DCMAKE_FIND_LIBRARY_SUFFIXES=".a" -DCMAKE_PREFIX_PATH="/opt/scopy/lib/cmake;$INSTALLED_DEPS" ..
make
...
echo "debian/scopy/opt/scopy/bin/decoders usr/local/scopy/bin/decoders" >> ${TRAVIS_BUILD_DIR}/debian/scopy.install
echo "debian/scopy/opt/scopy/bin/qt.conf usr/local/scopy/bin/qt.conf" >> ${TRAVIS_BUILD_DIR}/debian/scopy.install
